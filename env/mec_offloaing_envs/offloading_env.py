...
class Resources(object):
    ...
    # 重置模拟器的方法，这里将 MEC 处理可用时间 和 移动处理可用时间重置为 0
    def reset(self):
        self.mec_process_avaliable_time = 0.0
        self.mobile_process_avaliable_time = 0.0
...
# OffloadingEnvironment 卸载环境 继承了 MetaEnv 类
class OffloadingEnvironment(MetaEnv):
    def __init__(self, resource_cluster, batch_size,
        ...
        # set the file path of task graphs.
        # 为每一个任务图设置文件路径
        self.graph_file_paths = graph_file_paths
    ...
    def greedy_solution(self):
        # 定义两个空列表，一个为 result_plan,一个为 finish_time_batchs
        result_plan = []
        finish_time_batchs = []
        ...
        for task_graph_batch in self.task_graphs_batchs:
            plan_batchs = []
            finish_time_plan = []
            # 遍历当前 task_graph_batch 中的每一个 task_graph
